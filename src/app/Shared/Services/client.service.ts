import { ChangeDetectorRef, inject, Injectable, signal } from '@angular/core';
import { HttpClient, HttpParams } from '@angular/common/http';
import { Router } from '@angular/router';
import { BehaviorSubject, Observable, of, throwError } from 'rxjs';
import { environment } from '../../../environments/environment';
import { User } from '../Models/User.model';
import { catchError, map, tap } from 'rxjs/operators';
import { FeatureAuth } from '../Models/FeatureAuth.model';
import { Pagination } from '../Models/Pagination.model';
import { FeatureActions } from '../enums/feature-actions';
import { Client } from '../Models/Client.model';

@Injectable({
  providedIn: 'root',
})
export class ClientService {
  /*endpointAuth = `${environment.api}/clients`;
  _Client = signal<Client | null>(null);
  private _statusTimeout: any;
  private _inactivityTimeout: any;
  _defaultLink = new BehaviorSubject<string | null>(null);
  features = signal<FeatureAuth[] | null>(null);
  http = inject(HttpClient);
  router = inject(Router);
  client = this._Client.asReadonly();

  private _trackActivity() {
    window.addEventListener('mousemove', this._resetInactivityTimer.bind(this));
    window.addEventListener('keydown', this._resetInactivityTimer.bind(this));
  }
  private _resetInactivityTimer() {
    clearTimeout(this._inactivityTimeout);
    if (this._Client()?.userId.status === 'away') {
      this._Client.update((u) => ({
        ...u,
        userId: {
          ...u?.userId,
          status: 'online',
        },
      }));
      console.log(this.client());
      this.updateState('online').subscribe();
    }
    this._inactivityTimeout = setTimeout(() => {
      this.updateState('away').subscribe();
    }, 300000); // 5 minutes inactivity
  }

  checkPermission(code: string, action: string): boolean {
    const featuresAuth = this.features();
    return !!featuresAuth?.some(
      (fau) =>
        fau.code === code && fau.actions?.includes(action as FeatureActions)
    );
  }

  get(): Observable<Client> {
    return this.http.get<Client>(`${this.endpointAuth}/me`).pipe(
      tap((user) => {
        this._Client.set(user);
      }),
      catchError((error) => {
        return throwError(() => error); // Propagate the error
      })
    );
  }
  getAll(): Observable<Client[]> {
    return this.http.get<Client[]>(`${this.endpointAuth}/all`);
  }

  changePassword(
    password: string,
    newPassword: string,
    code: string,
    id: string
  ): Observable<null> {
    return this.http.post<null>(`${this.endpointAuth}/${id}/change-password`, {
      password,
      newPassword,
      code,
    });
  }
  checkPassword(password: string): Observable<null> {
    return this.http.post<null>(`${this.endpointAuth}/check-password`, {
      password,
    });
  }
  sendCode(
    id: string,
    password: string,
    newPassword: string
  ): Observable<null> {
    return this.http.post<null>(`${this.endpointAuth}/${id}/send-code`, {
      password,
      newPassword,
    });
  }
  resendCode(id: string, params: any): Observable<null> {
    return this.http.patch<null>(`${this.endpointAuth}/${id}/resend-code`, {
      params,
    });
  }
  addUser(user: Client): Observable<Client> {
    return this.http.post<Client>(`${this.endpointAuth}`, {
      user,
    });
  }
  getUsers(
    limit: string,
    page: string,
    search: string,
    filterGroups: string,
    filterStatus: string,
    filterNewOld: string
  ): Observable<Pagination<Client>> {
    let searchParams = new HttpParams();
    searchParams = searchParams.append('limit', limit);

    searchParams = searchParams.append('page', page);

    if (search) {
      searchParams = searchParams.append('search', search);
    }
    if (filterGroups) {
      searchParams = searchParams.append('filtergroups', filterGroups);
    }
    if (filterStatus) {
      searchParams = searchParams.append('filterStatus', filterStatus);
    }
    if (filterNewOld) {
      searchParams = searchParams.append('filterNewOld', filterNewOld);
    }
    return this.http.get<Pagination<Client>>(`${this.endpointAuth}`, {
      params: searchParams,
    });
  }
  getOne(id: string): Observable<Client> {
    return this.http.get<Client>(`${this.endpointAuth}/${id}`);
  }
  updateOne(user: User): Observable<Client> {
    return this.http.put<Client>(`${this.endpointAuth}/${user._id}`, { user });
  }
  enableAccount(id: string): Observable<Client> {
    return this.http.get<Client>(`${this.endpointAuth}/${id}/enable-disable`);
  }
  deleteOne(id: string): Observable<null> {
    return this.http.delete<null>(`${this.endpointAuth}/${id}`);
  }
  updateAvatar(data: FormData, id: string): Observable<Client> {
    return this.http.post<Client>(`${this.endpointAuth}/${id}/avatar`, data).pipe(
      tap((user) => {
        this._Client.set(user);
      })
    );
  }

  initializeUser(user: Client) {
    this._Client.set(user);
    this._trackActivity();
  }
  updateState(status: string): Observable<Client> {
    console.log('hello service');
    return this.http
      .patch<Client>(`${this.endpointAuth}/status`, { status })
      .pipe(
        tap((updatedUser) => {
          this._Client.update((u) => ({ ...u!, status }));
        })
      );
  }*/
}
