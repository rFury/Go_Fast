...
export class OrdersComponent implements OnInit, OnDestroy {
  ...
  startJourney(): void {
    ...
    const sub = this._agentService.startJourney().subscribe({
      next: (response) => {
        if (response.orders?.length) this.processJourneyOrders(response.orders);
...
private processJourneyOrders(orders: Order[]): void {
  ...
  this.stops = orders
    .map((order): PendingRouteStop | null => {
      if (order.status === Status.assigned && order.pick_up?.place?.coordinates) {
        return { order, coordinates: order.pick_up.place.coordinates, isPickup: true, status: "pending" };
      } else if (order.status === Status.picked_up && order.destination?.place?.coordinates) {
        return { order, coordinates: order.destination.place.coordinates, isPickup: false, status: "pending" };
      }
      return null;
    })
  ...
  private addStopMarker(stop: RouteStop, index: number): void {
    ...
    const iconClass = stop.status === 'completed' ? 'bg-green-500' : stop.status === 'active' ? 'bg-blue-500 animate-pulse' : stop.isPickup ? 'bg-yellow-500' : 'bg-red-500';
    el.innerHTML = `<div class="flex items-center justify-center rounded-full ${iconClass} text-white w-8 h-8 shadow-lg">${index + 1}</div>`;
    ...
    popup.on('open', () => {
      setTimeout(() => {
        const btn = document.querySelector(`.view-details-btn[data-order-index="${index}"]`);
        if (btn) btn.addEventListener('click', () => this.openOrderDetails(stop.order));
      }, 100);
    });
    const marker = new mapboxgl.Marker({ element: el }).setLngLat(stop.coordinates).setPopup(popup).addTo(this.map!);
    this.markers.push(marker);
  }

  private createFullRoute(): void {
    if (this.stops.length < 2) return;
    const waypoints = this.stops.map((stop) => `${stop.coordinates[0]},${stop.coordinates[1]}`).join(';');
    fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${waypoints}?geometries=geojson&overview=full&access_token=${this._mapService.mapboxToken}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  private addRouteToMap(geometry: any, id: string, color: string, width: number): string {
    const sourceId = `${id}-${Date.now()}`;
    this.routeSources.push(sourceId);
    if (!this.map!.isStyleLoaded()) {
      this.map!.once('styledata', () => this.addSourceAndLayer(sourceId, geometry, color, width));
    } else {
  ...
  private addSourceAndLayer(sourceId: string, geometry: any, color: string, width: number): void {
    this.map!.addSource(sourceId, { type: 'geojson', data: { type: 'Feature', properties: {}, geometry } });
    this.map!.addLayer({
      id: sourceId,
      type: 'line',
      source: sourceId,
  ...
  private centerMapOnCurrentStop(): void {
    ...
    this.map!.flyTo({ center: currentCoords, zoom: 15, pitch: 45, bearing: 0, duration: 1000 });
  }
  ...
  private async getCurrentPosition(): Promise<[number, number]> {
    ...
    const options: PositionOptions = { enableHighAccuracy: true, timeout: 60_000, maximumAge: 5_000 };
    try {
  ...
  async startNavigation(simulationMode = false): Promise<void> {
    ...
    try {
      ...
      if (simulationMode) {
        this.userLocation = this.currentStopIndex === 0 ? this.stops[0].coordinates : this.stops[this.currentStopIndex - 1].coordinates;
  ...
  private setupSimulatedLocationUpdates(): void {
    ...
    this.cumulativeDistances = this.computeCumulativeDistances(this.currentLegCoordinates);
    ...
    this.simulationSubscription = interval(100).subscribe(() => {
      this._ngZone.run(() => {
        ...
        const distanceTraveled = (this.simulatedSpeedKmh / 3600) * elapsedSeconds;
        const position = this.getPositionAtDistance(distanceTraveled);
        if (position) {
          const nextPosition = this.getPositionAtDistance(distanceTraveled + 0.01);
          const heading = nextPosition ? this.calculateBearing(position, nextPosition) : this.currentBearing;
          this.updateUserLocation(position, heading);
          if (distanceTraveled >= totalDistance) this.simulationSubscription?.unsubscribe();
        }
      });
    });
  }

  private computeCumulativeDistances(coordinates: [number, number][]): number[] {
  ...
  private setupLocationTracking(): void {
    ...
    Geolocation.watchPosition({ enableHighAccuracy: true, timeout: 60000, maximumAge: 5000 }, (position, error) => {
      if (position) {
        this._ngZone.run(() => {
          const newLocation: [number, number] = [position.coords.longitude, position.coords.latitude];
          const heading = this.previousLocation && position.coords.speed && position.coords.speed > 1
  ...
  private updateUserLocation(location: [number, number], heading: number): void {
    ...
    if (!this.userLocationMarker) {
      ...
      this.userLocationMarker = new mapboxgl.Marker({ element: el, rotationAlignment: 'map' }).setLngLat(snappedLocation).addTo(this.map!);
    } else {
  ...
  private updateDirectionArrow(heading: number): void {
    if (!this.directionArrow) {
      ...
      this.directionArrow = new mapboxgl.Marker({ element: el, rotationAlignment: 'map', anchor: 'center' }).setLngLat(this.userLocation!).addTo(this.map!);
    }
    this.currentBearing = heading;
    this.directionArrow.setLngLat(this.userLocation!);
    const arrowEl = this.directionArrow.getElement().querySelector('#direction-arrow');
    if (arrowEl) arrowEl.setAttribute('style', `transform: rotate(${heading}deg); display: flex; align-items: center; justify-content: center;`);
  }

  private updateCameraPosition(location: [number, number], heading: number): void {
    this.map!.jumpTo({ center: location, zoom: 17, pitch: this.map!.getPitch(), bearing: heading });
  }

  private checkDistanceToCurrentStop(location: [number, number]): void {
    if (this.currentStopIndex >= this.stops.length) return;
    const distance = this.calculateDistance(location, this.stops[this.currentStopIndex].coordinates);
  ...
  private calculateDistance(point1: [number, number], point2: [number, number]): number {
    ...
    const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(toRad(point1[1])) * Math.cos(toRad(point2[1])) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
    return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  }

  private showArrivalNotification(): void {
    this._snackBar.open('You have arrived at the destination!', 'Complete Stop', { duration: 0 }).onAction().subscribe(() => this.completeCurrentStop());
  }
  ...
  private addDestinationMarker(stop: RouteStop): void {
    ...
    const marker = new mapboxgl.Marker({ element: el }).setLngLat(stop.coordinates).addTo(this.map!);
    this.markers.push(marker);
  }
  ...
  private updateRouteToFollowUser(location: [number, number]): void {
    ...
    if (currentIndex !== -1) {
      const remainingCoordinates = this.currentLegCoordinates.slice(currentIndex);
      const source = this.map!.getSource(this.activeRouteSourceId) as mapboxgl.GeoJSONSource;
      source.setData({
        type: 'Feature',
        properties: {},
  ...
  private checkStepCompletion(location: [number, number]): void {
    if (!this.currentStep || !this.navigationSteps.length || !this.currentStep.location) return;
    const currentStepIndex = this.navigationSteps.indexOf(this.currentStep);
    if (currentStepIndex === -1) return;
    const distanceToStep = this.calculateDistance(location, this.currentStep.location);
    if (distanceToStep < 0.05 && currentStepIndex < this.navigationSteps.length - 1) {
  ...
  private getPositionAtDistance(distance: number): [number, number] | null {
    ...
    if (distance >= totalDistance) return this.currentLegCoordinates[this.currentLegCoordinates.length - 1];
    let i = 1;
    while (i < this.cumulativeDistances.length && this.cumulativeDistances[i] < distance) i++;
    ...
    return [prevPoint[0] + fraction * (nextPoint[0] - prevPoint[0]), prevPoint[1] + fraction * (nextPoint[1] - prevPoint[1])];
  }
  ...
  private calculateRouteBounds(coordinates: [number, number][]): mapboxgl.LngLatBounds {
  ...
  getManeuverIcon(type: string): string {
    const iconMap: { [key: string]: string } = {
      turn: 'turn_right', straight: 'straight', 'slight right': 'turn_slight_right', right: 'turn_right',
      'sharp right': 'turn_sharp_right', uturn: 'u_turn_right', 'slight left': 'turn_slight_left', left: 'turn_left',
      'sharp left': 'turn_sharp_left', roundabout: 'roundabout_right', 'exit roundabout': 'roundabout_right',
      fork: 'fork_right', merge: 'merge', ramp: 'ramp_right', exit: 'exit_to', arrive: 'location_on', depart: 'my_location',
    };
  ...
  get upcomingSteps(): NavigationStep[] {
    ...
    return currentIndex === -1 ? [] : this.navigationSteps.slice(currentIndex + 1, currentIndex + 4);
  }
  ...
  toggleNavigationView(): void {
    ...
    this.map.easeTo({ pitch: targetPitch, bearing: targetBearing, duration: 1000, easing: (t) => t });
    this.add3DBuildingsLayer();
  }

  showUpcomingTurns(): void {
    if (!this.navigationSteps.length) return;
    const currentStepIndex = this.navigationSteps.findIndex((step) => step === this.currentStep);
    if (currentStepIndex === -1) return;
    const upcomingTurns = this.navigationSteps.slice(currentStepIndex + 1, currentStepIndex + 4);
    console.log('Upcoming turns:', upcomingTurns);
  }

  private startRouteUpdateInterval(): void {
    this.routeUpdateInterval = setInterval(() => {
      if (this.isNavigating && !this.forcedSimulationMode) this.updateNavigationToNextStop();
    }, 10000);
  }
}